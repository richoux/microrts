...
//                              if (DEBUG>=1) {System.out.println("repaint done.");out.flush();}
	...
	public static void runExperiments( List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations, int max_cycles, int max_inactive_cycles, boolean visualize,
			...
			for( int ai2_idx = 0; ai2_idx < bots.size(); ++ai2_idx ) 
			{
				...
				{            
					for( int i = 0; i < iterations; ++i )
					{
						...
						} while( !gameover && gs.getTime() < max_cycles && gs.getTime() - lastTimeActionIssued < max_inactive_cycles );
						...
						if( saveTrace )
						{
							...
							if( saveZip )
							{
								zip = new ZipOutputStream( new FileOutputStream( "replays_micro_rts" + filename + ".zip" ) );
								zip.putNextEntry( new ZipEntry( "game.xml" ) );
								xml = new XMLWriter( new OutputStreamWriter( zip ) );
							}
							else
